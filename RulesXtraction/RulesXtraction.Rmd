...
Aunque es tentador seleccionar aquellas reglas que tienen mayor soporte y confianza, situadas a la derecha en la gráfica, hay que tener el valor de lift que está próximo a 1, el valor 1 para lift significa independencia estadística por lo que los items que se relacionan en estos casos no tienen una relación de causalidad. Es por esto que hacemos otro refinamiento de las reglas obtenidas para dejar fuera este conjunto y nos quedamos con las reglas con lift mayor a 1.5.
\newpage
```{r}
rulesInteresting <- subset(rulesConfidenceInteresting,subset =  lift > 1.5 )
inspect(head(rulesInteresting))
```
